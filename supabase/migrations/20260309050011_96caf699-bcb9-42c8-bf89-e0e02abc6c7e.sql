...
CREATE POLICY "Members can read their trackers" ON public.trackers FOR SELECT TO authenticated USING (public.is_tracker_member(auth.uid(), id));
...
CREATE POLICY "Admin can insert members" ON public.tracker_members FOR INSERT TO authenticated WITH CHECK (public.is_tracker_admin(auth.uid(), tracker_id) OR user_id = auth.uid());
CREATE POLICY "Members can view tracker members" ON public.tracker_members FOR SELECT TO authenticated USING (public.is_tracker_member(auth.uid(), tracker_id));
CREATE POLICY "Admin can update member roles" ON public.tracker_members FOR UPDATE TO authenticated USING (public.is_tracker_admin(auth.uid(), tracker_id));
CREATE POLICY "Members can delete own membership or admin can remove" ON public.tracker_members FOR DELETE TO authenticated USING (user_id = auth.uid() OR public.is_tracker_admin(auth.uid(), tracker_id));
...
CREATE POLICY "Read system or tracker categories" ON public.categories FOR SELECT TO authenticated USING (is_system = true OR public.is_tracker_member(auth.uid(), tracker_id));
CREATE POLICY "Members can insert custom categories" ON public.categories FOR INSERT TO authenticated WITH CHECK (tracker_id IS NOT NULL AND public.is_tracker_member(auth.uid(), tracker_id));
CREATE POLICY "Members can update custom categories" ON public.categories FOR UPDATE TO authenticated USING (tracker_id IS NOT NULL AND public.is_tracker_member(auth.uid(), tracker_id));
CREATE POLICY "Creator or admin can delete custom categories" ON public.categories FOR DELETE TO authenticated USING (tracker_id IS NOT NULL AND (created_by = auth.uid() OR public.is_tracker_admin(auth.uid(), tracker_id)));
...
CREATE POLICY "Members can read tracker expenses" ON public.expenses FOR SELECT TO authenticated USING (public.is_tracker_member(auth.uid(), tracker_id));
CREATE POLICY "Members can insert expenses" ON public.expenses FOR INSERT TO authenticated WITH CHECK (public.is_tracker_member(auth.uid(), tracker_id) AND created_by_id = auth.uid());
CREATE POLICY "Creator or admin can update expenses" ON public.expenses FOR UPDATE TO authenticated USING (created_by_id = auth.uid() OR public.is_tracker_admin(auth.uid(), tracker_id));
CREATE POLICY "Creator or admin can delete expenses" ON public.expenses FOR DELETE TO authenticated USING (created_by_id = auth.uid() OR public.is_tracker_admin(auth.uid(), tracker_id));
