...
CREATE POLICY "Authenticated users can create trackers" ON public.trackers
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (admin_id = auth.uid());

CREATE POLICY "Members can read their trackers" ON public.trackers
  AS PERMISSIVE FOR SELECT TO authenticated USING (is_tracker_member(auth.uid(), id));

CREATE POLICY "Admin can update tracker" ON public.trackers
  AS PERMISSIVE FOR UPDATE TO authenticated USING (admin_id = auth.uid());

CREATE POLICY "Admin can delete tracker" ON public.trackers
  AS PERMISSIVE FOR DELETE TO authenticated USING (admin_id = auth.uid());
...
CREATE POLICY "Admin can insert members" ON public.tracker_members
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_tracker_admin(auth.uid(), tracker_id) OR (user_id = auth.uid()));

CREATE POLICY "Members can view tracker members" ON public.tracker_members
  AS PERMISSIVE FOR SELECT TO authenticated USING (is_tracker_member(auth.uid(), tracker_id));

CREATE POLICY "Admin can update member roles" ON public.tracker_members
  AS PERMISSIVE FOR UPDATE TO authenticated USING (is_tracker_admin(auth.uid(), tracker_id));

CREATE POLICY "Members can delete own membership or admin can remove" ON public.tracker_members
  AS PERMISSIVE FOR DELETE TO authenticated USING ((user_id = auth.uid()) OR is_tracker_admin(auth.uid(), tracker_id));
...
CREATE POLICY "Read system or tracker categories" ON public.categories
  AS PERMISSIVE FOR SELECT TO authenticated USING ((is_system = true) OR is_tracker_member(auth.uid(), tracker_id));

CREATE POLICY "Members can insert custom categories" ON public.categories
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((tracker_id IS NOT NULL) AND is_tracker_member(auth.uid(), tracker_id));

CREATE POLICY "Members can update custom categories" ON public.categories
  AS PERMISSIVE FOR UPDATE TO authenticated USING ((tracker_id IS NOT NULL) AND is_tracker_member(auth.uid(), tracker_id));

CREATE POLICY "Creator or admin can delete custom categories" ON public.categories
  AS PERMISSIVE FOR DELETE TO authenticated USING ((tracker_id IS NOT NULL) AND ((created_by = auth.uid()) OR is_tracker_admin(auth.uid(), tracker_id)));
...
CREATE POLICY "Members can insert expenses" ON public.expenses
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (is_tracker_member(auth.uid(), tracker_id) AND (created_by_id = auth.uid()));

CREATE POLICY "Creator or admin can update expenses" ON public.expenses
  AS PERMISSIVE FOR UPDATE TO authenticated USING ((created_by_id = auth.uid()) OR is_tracker_admin(auth.uid(), tracker_id));

CREATE POLICY "Creator or admin can delete expenses" ON public.expenses
  AS PERMISSIVE FOR DELETE TO authenticated USING ((created_by_id = auth.uid()) OR is_tracker_admin(auth.uid(), tracker_id));
...
CREATE POLICY "Authenticated can update category_learning" ON public.category_learning
  AS PERMISSIVE FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL);
